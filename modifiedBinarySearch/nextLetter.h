...
/* url https://www.designgurus.io/course-play/grokking-the-coding-interview/doc/639f23b94e4f288d4a83ae0d */
...
public:
    static char searchNextLetter(const vector<char>& letters, char key)
    {
        ...
        {
            int mid = (beg + end) / 2;
            if (letters[mid] > key && mid > 0 && letters[mid-1] <= key) return letters[mid];
            if (letters[mid] <= key)
            {
                beg = mid + 1;
            }
